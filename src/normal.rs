            ...
            n => {
                self.mean
                    * Self {
                    ...
                    .expectation_value()
                    + T::from_usize(n - 1).expect("must fit in `T`")
...
impl<T: Float + FromPrimitive + std::fmt::Display + std::fmt::Debug> Covariance<T> for Normal<T> {
    fn off_diagonal_covariance(&self, other: &Self) -> T {
        if (self.mean == other.mean) && (self.standard_deviation == other.standard_deviation) {
            ...
            .expectation_value()
                - self.expectation_value() * other.expectation_value()
        } else {
            unimplemented!()
        }
    }
        ...
        Ok(Self {
            distributions: distributions
                ...
                .map(|(distribution, unnormalized_weight)| WeightedNormal {
                    distribution,
                    weight: unnormalized_weight / normalization,
                })
                .collect(),
        })
    }
...
impl<T: Float + FromPrimitive + std::fmt::Display + std::fmt::Debug> Distribution<T> for Mixture<T> {
    ...
    fn expectation_value(&self) -> T {
        self.distributions
            .iter()
            .map(|weighted_distribution| {
                weighted_distribution.weight
                    * weighted_distribution.distribution.expectation_value()
            })
            .fold(T::zero(), |a, b| a + b)
    }
    ...
    fn variance(&self) -> T {
        ...
        let expectation_value_of_square = self
            .distributions
            .iter()
        ...
        let square_of_expectation_value = self
            .distributions
            .iter()
            .map(
                |WeightedNormal {
...
mod tests {
    ...
    fn off_diagonal_covariances_of_normal_distribution_with_powers_return_correct() {
        ...
        for (power, expected) in powers.into_iter().zip(expected_results.into_iter()) {
            let other_distribution = Normal {
                mean,
                standard_deviation,
                power,
            };
            approx::assert_relative_eq!(
                normal.off_diagonal_covariance(&other_distribution),
                expected
            );
        }
    }
    ...
    fn mixture_of_equal_distributions_retains_properties() {
        ...
        let mixture: Mixture<f64> =
            Mixture::from_distributions_and_weights(distributions, weights.clone()).unwrap();

        approx::assert_relative_eq!(mixture.expectation_value(), mean,)
    }
    ...
    fn mixture_of_normals_has_correct_expectation_value() {
        ...
        let distributions = means
            ...
            .map(|(&mean, &standard_deviation)| Normal {
                mean,
                standard_deviation,
                power: 1,
            })
